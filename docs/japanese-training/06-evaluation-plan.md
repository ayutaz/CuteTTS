...
### CERには約10%の床がある（2026-08-31実測）

**人間の実音声**を同じASR・同じ正規化で測ると CER mean 10.4% / median 6.7% が出る
（gol原音声80件。[RESULTS.md](RESULTS.md)）。**TTSのCERを0%基準で読んではならない。**

| 対象 | 素CER | **読みCER** |
|---|---:|---:|
| 人間の実音声（誤り床） | 10.42% | **5.59%** |
| S0学習後のTTS | 28.4% | 未測定 |
| **fp32 30,000 step（v3 600文）** | **20.10%** | **13.38%** |
| 同（**J2+J3込み＝実運用**） | 19.71% | **12.36%** |
| **TTS由来の誤り** | 9.68pt | **7.79pt**（実運用 6.77pt） |

ゲートを引くときは床を明示する。「CER 10%を目指す」は素CERでは床とほぼ同義で、
実質達成不能。

### 読みCERを併記する（R-029、2026-09-13実測）

**ASRは音声を聞いて自分で表記を選ぶ。** `なにも` と話しても `何も` と書く。
参照文と表記が違えば素のCERは誤りと数えるが、**発音は合っている**。
人間の実音声の床 10.42% のうち **4.83pt がこれ**だった（読みへ直すと 5.59%）。

| 参照文 | ASR転写 | 素CER | 読みCER |
|---|---|---:|---:|
| 綺麗さっぱり | キレイさっぱり | 50.0% | **0.0%** |

**仮名を入力に含む比較（J2 / J3）では読みCERを主に読む**（D-038）。
J3 の効果は素CERで -2.64pt、読みCERで **-4.23pt**。素CERは過小評価していた。

**置き換えるものではない。** 同音異義（`聞く` / `効く`）の誤りは見えなくなるので、
**必ず両方を出す**。`evaluate_japanese_cer.py` は `cer` と `cer_reading` を記録する。
実装は `cutetts.training.yomi.reading_form`（`pyopenjtalk` が要る）。
