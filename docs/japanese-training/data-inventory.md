...
|---|---:|
| 総行数 | 7,405,094 |
| game数（metadata上） | 596（tarは602。差は大きいgameの `_partN` 分割・下記参照） |
| 話者ID数 | 19,349 |
...
**大きいgameは `<game_id>_part1.tar` / `<game_id>_part2.tar` に分割されている。**
tar 602本に対し metadata 上の game が 596 なのはこのため。

```text
9381931FAB68786161D5A740F32C5A33_part1.tar   # 93.0h のgameが2本に分かれる
9381931FAB68786161D5A740F32C5A33_part2.tar
F1C9239826100C845CC7FB342E0C8374.tar         # 61.7h は1本
```

**tarのファイル名をそのまま game_id として扱ってはならない。** S1の選定では
5 game中2 game（170時間・全体の52%）がエラーも出さずにmanifestから落ちていた。
`prepare_japanese_manifest.py` は `_partN` を除いて game 単位へまとめ、
各partのwavを走査して `audio_ref` が実際にwavを含むtarを指すようにする
（`tests/training/test_gol_split_tars.py` で固定）。

### ゲーム単位の分布（metadata.tsv 全7.4M行を集計、2026-09-01）

tarがgame単位なので、**どのgameを落とすかを事前に設計できる**。
集計結果は `data/gol_game_stats.json`（596 game分の時間・話者数・text性質）。

| 項目 | 値 |
|---|---:|
| 最大game | 93.0 h / 249話者 |
| 10時間以上のgame | 434本 |
| **数字を含む文の割合** | **1.3%**（10h以上のgameでも中央値0.9%、最大5.9%） |

**数字・固有名詞の読みはデータ量では学習できない。** これはS0で
out_of_domain CER が改善しなかったこと（74.7% → 76.4%）の直接の説明であり、
S1のゴールから外す根拠になった（D-026）。

### S1で選定した5 game

| game_id（先頭12桁） | 時間 | 話者 | GB | 数字文% |
|---|---:|---:|---:|---:|
| 9381931FAB68 | 93.0 | 249 | 61.3 | 3.7 |
| AA538DEF78C6 | 76.9 | 215 | 50.6 | 0.1 |
| F1C923982610 | 61.7 | 140 | 40.6 | 2.3 |
| 2A8DB5A70357 | 52.2 | 166 | 34.4 | 1.2 |
| F4736E42EB29 | 42.2 | **427** | 27.8 | 2.7 |
| **計** | **326.0** | **1,197** | **214.8** | |

実ファイルは分割により7本。前処理後の accepted は 196,606発話 / 318.6時間。
