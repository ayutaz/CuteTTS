...
| D-026 | out_of_domain（数字・固有名詞）をS1のゴールに含めない | **決定済み** | ユーザー判断（2026-09-01）。golのcorpusで数字を含む文は1.3%（10h以上の434ゲームでも中央値0.9%）で、データ量では解決しない。S1ではD-008の読み誤り内訳の集計のみ行い、対処はS2で決める |
...
`out_of_domain` は 74.7% → 71.4% でほぼ動かず、D-026（数字・固有名詞はデータ量で
直らない）を裏づけている。
