...
**対照実験**（同一データ・同一step・同一seed、`--param-dtype` のみ変更）:

| 実行 | backboneが動いた割合 | in_domain mean / median | 打切 |
|---|---:|---:|---:|
| `s1v2-3000`（bf16・従来） | 約 9% | 30.69 / 27.52 | 1/30 |
| **`s1v2-fp32-3000`** | **100%** | **24.36 / 23.23** | **0/30** |

差 **-6.33pt**、95%CI [-11.13, -1.61]、**有意**。base 比 -11.42pt [-17.80, -5.50]。
**24.36% は全24実行の最良**で、S0（28.36%）を 4.00pt 下回る（ただし n=30 では有意差なし）。
