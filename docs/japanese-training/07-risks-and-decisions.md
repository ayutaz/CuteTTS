...
**対策（D-032）**: S2 で中国語能力を保つなら、次のいずれかが要る。

- **replay 混合**（D-009 の 5〜10%）。最も素直だが、日本語データを減らす
- **言語条件づけ**。prompt に言語指示を入れて読みを分岐させる。
  ただし `processor` の prompt テンプレートは推論側と対で動くので、
  変えるなら両方を同時に変える（CLAUDE.md「prompt textは英語のinstruction固定」）
- **中国語を諦める**。日本語特化modelと位置づける

**S1 の時点では判断しない。** どれを採るかは「このmodelの用途に中国語が要るか」で
決まり、それは技術ではなく製品の判断（R-009 の public/private 境界と同じ層）。
