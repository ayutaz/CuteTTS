...
| ~~3~~ | ~~M4h~~ | 韻律転写を**本番データで**回す（325.9h / `--f0-dropout 0.1`） | 要 | 約12.5h / 約$2.2 | **完了。3つの基準すべて達成**（R-060）。利得 +0.104 / 読みCER 7.12% / 代償 -0.025。**追随度はデータ量で上がった**（+0.126 → +0.168） |
| **4** | S2 | 1,000時間 | 要 | 大 | 読みで -1.5pt。**抑揚は追随度だけ上がる**（対数的） |

**M4 は完了した。** 韻律転写は機能として出せる（checkpoint は
`checkpoints/m4h-prosody/inference/`）。**アクセント核だけは最後まで動かなかった。**
...
#### 結果: **3つとも達成**（2026-09-20。R-060）

| 指標 | 基準 | 実測 | 判定 |
|---|---|---|---|
| 輪郭の追随（mismatch → oracle） | +0.08 以上（有意） | **+0.104** [+0.062, +0.146] | **達成** |
| 読みCER | 7.58% から +1pt 以内 | **7.12%** [-1.24, +0.25] | **達成** |
| 条件なしの輪郭 | 素（+0.091）から -0.04 以内 | **+0.067** [-0.062, +0.012] | **達成** |

**追随度はデータ量で上がった**（17.9h +0.126 → 325.9h **+0.168**、
95%CI [+0.005, +0.079]、有意）。上限 +0.367 に対して 34% → **46%**。

**アクセント核は動かない**（oracle 43.5% 対 条件なし 43.1%、有意差なし）。
間違った F0 は壊す（-3.78pt、有意）ので**特異性はある**が、
正しくしても良くならない。
...
**疑っていること**: S2 の根拠になっている外挿 -1.5pt は、**読みCER 13.38% の
水準で測った傾き**（D2 / R-042。17.9h 17.20% → 80.1h 15.22% → 325.9h 13.38%、
**-3.0pt/10倍**、frontend 無し）。

**ところが C2 で「計算量は 7.58% の水準では効かない」と分かっている**
（13.38% では -1.40pt 有意 → 7.58% では -0.33pt 有意差なし。R-055 / D-053）。
**データ量も同じく飽和している可能性がある。**

**方法**: 17.9h と 80h を **`--frontend accent`** で 30,000 step 学習し
（batch 4 / lr 2e-5 / seed 42 — `m4a-accent` と完全に同条件）、
読みCER を評価set v3（600文）で測る。**325.9h の点は `m4a-accent` の
7.58% をそのまま使う**（D1 / D2 も既存の基準線を再利用している）。
部分集合は `build_data_subset.py` の既定 seed なので D1 / D2 と同一。

**判定**（**結果を見る前に決める**）:

| 結果 | 判断 |
|---|---|
| 17.9h → 325.9h（18倍）の差が **有意** | 傾きは残っている → **S2 へ進む**（P1e Pass B の見積もりを出す） |
| **有意差なし** | **読みは完了と宣言し、S2 を取り下げる** |

`ParameterDrift` が全module 100% であることを毎run確認する（R-020）。

**見積もり**: 学習 1.4h × 2 + 読みCER評価 約1h × 2 = **約5時間 / 約$0.8**
（RTX 3090 / $0.1543 per hour）。**実測は学習 1.39h / 1.42h。**

### 結果: **有意差なし → 読みは閉じた**（R-061 / D-055）

| データ量 | frontend 無し（D2） | **`accent`（D3）** |
|---|---:|---:|
| 17.9h | 17.20% | **8.17%** |
| 80.1h | 15.22% | **7.75%** |
| 325.9h | 13.38% | **7.58%** |

**18倍で -0.59pt、95%CI [-1.41, +0.27]。有意差なし。**
傾きは -3.0 → **-0.47pt/10倍**。

**信頼区間の最も都合のよい端でも S2 は床に届かない** —
1,000h で -0.55pt（7.03%）、床には約19,300時間が要る。

**S2 を取り下げる**（D-055）。

## J5: **frontend 自身の読み誤り**（手がかり。未測定）

**D3 で「規模では動かない」と分かった約2.0pt の中身の候補。**

聴取キットを作るときに目視で見つけた。`--frontend accent` は
**pyopenjtalk の読みをそのまま信じる**ので、**G2P が誤ると
モデルは忠実に誤った読みを喋る。**

```
ただの誤作動として片付けて → タ'ダノアヤマサドートシテカタズ'ケテ
                                  ^^^^^^^^^^ 正しくは ゴサドー
作動を確認                 → サドーーカクニン
                                  ^^^ 長音記号が重なっている
```

`誤作動` が `誤る`(アヤマ) + `作動`(サドー) に分解されている。
**M4a より前は入力が漢字だったので、モデルが正しく読める可能性があった。**
片仮名化はその可能性を消す代わりに -4.40pt を得た取引だった。

**ここは未測定である。** 見つけたのは目視の2例で、
**頻度も、読みCER への寄与も分かっていない。**

### なぜすぐ測れないか

**`reading_form()` は片仮名を入れると壊れる**（実測）。

```
エー、ショクブツノーンシツデス → えしょくぶつの     # 大半が消える
ジーサンヤジーヨンワ、サイセ… → さいせ
```

素朴に「frontend の片仮名」と「実際の読み」を比べると **CER 132%** という
無意味な値が出る。**長音記号の展開**（`セー` は `せい` か `せえ` か）と
**ラテン文字の扱い**（`reading_form` は落とす／frontend は `ジー` と読む）が
揃っていないため。**比較のための正規化を先に作る必要がある。**

### 測り方の案（CPU のみ）

人間の実音声80文（`artifacts/asr-floor/`）には ASR の書き起こしがある。
これを「**実際にどう読まれたか**」として、

* (a) 元テキストの読み 対 実際の読み ← いまの床 5.59%
* (b) **frontend の片仮名** 対 実際の読み

を同じ正規化の上で比べる。**(b) が有意に遠ければ、frontend が
読みを持ち込んでいる**ことになり、直せば読みCER が動く。
...
**`--frontend accent` の水準ではデータ量の傾きが約1/6になり、
18倍にしても有意差が出なかった**（-0.59pt、95%CI [-1.41, +0.27]）。
**信頼区間の最も都合のよい端で計算しても** 1,000h は -0.55pt にしかならず、
人間の床 5.59% までなお 1.44pt 残る。床に届くには約19,300時間が要り、
**gol corpus 全体（10,466h）でも足りない。**

**抑揚には元から効かない**（D1 / R-038）ので、S2 はどちらの目的にも答えない。
以下は取り下げ前の検討として残す。

### （参考）保留時の記述（2026-09-16）
