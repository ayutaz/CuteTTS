...
> **これは S0 時点（2026-08-31）の記録で、数値は当時のまま凍結してある。**
> その後の到達点は [RESULTS.md](RESULTS.md) を見ること。
> in_domain CER は評価set v3（600文）で **20.10%**（base 35.86%）まで下がった。
> ここに書かれた 28.4% は v2（30文）での値で、**v2 の検出限界は 6.9pt** である
> （[R-020](07-risks-and-decisions.md) / n=30 の検出力問題）。
