...
S1のデータを **話者数だけ 733 → 88 に絞る**と、CER は 36.8% → **31.3%**（-5.5pt）。
データの中身（gol+moe）も前処理も同じで、規模だけ変えた比較。
phonetic も 47.5% → 34.6% と改善する。

**S1は「広く浅く」学習していた。** 733話者を1〜7回ずつしか見ておらず、
どの話者にも適応しきれない。S0は65話者を最大33回繰り返し、
少数話者へ十分に適応していた。

20,000 step でも 37.0% に留まったのはこれで説明できる。
step を10倍にしても、1話者あたりでは S0 の3,000 step の 59%（109 vs 185）にしかならない。
