...
| frontend | **J2（漢数字の読み展開）まで実施**。数詞set 200文で -11.80pt、**再学習不要**。J3（語の読み付与）は `pyopenjtalk-plus` で着手予定（D-035） |
| split | `split_group_id`（単連結）単位。zero-shot は **119クラスタ**（R-013は解消） |

**J2（漢数字の読み展開）は実施済み・有効**（2026-09-03）。
`千二百八十円` → `せんにひゃくはちじゅう円` と展開するだけで、数詞専用の評価set
200文で **-11.80pt**（95%CI [-17.45, -6.00]、有意）。通常の会話文600文では
+0.02pt で副作用なし。**再学習を要しない** — 学習コーパスに複合漢数字は0.32%しか
ないが、仮名ならモデルが既に読める。`src/cutetts/training/reading.py`。

**J3（語の読み付与）が次**。誤読は聴取4回すべてで最多の指摘（45%）で、
...
**J3でも直らないもの**: 作品固有のキャラクター名（`藤宮高邦` `聖` `珠音`）。
`聖` を一般語として `キヨシ` と読むのは正しく、人名で `せい` と読むかは文脈では
決まらない。ユーザー辞書が必要で、別作業として切り分ける。
