...
**実運用（J2+J3込み）は 12.36%。** 評価スクリプトは既定で frontend を
適用しないので、checkpoint比較の値と実運用の値は別に読む（F1）。
| ASR床（人間の実音声） | 10.40 |
...
| **G1** | 停止の健全性を指標化する | **次に着手**（GPU不要）。試聴9文中4文で文末の繰り返しが出た |
| **M3** | 抑揚に効く手段を設計する | **次**（GPU不要）。天井の68%が未達だが、試した軸すべてで動かなかった |
| F2 | frontend の音への効果 | 提案。R-039 の修正と J3 の副作用を測る |
| D2 | データ量の3点目（80h） | 提案。傾きが一定かを確かめる（S2 の前提） |
| ~~M2~~ | 抑揚・アクセントの天井 | **完了。天井は 輪郭 +0.38 / アクセント 64.5%**。現行は隔たりの約32%で**伸びしろが残っている** |
| ~~D1~~ | データ量の再測定（30,000 step） | **完了。読みCER -3.82pt で梃子だった**。ただし**抑揚は動かない** |
| ~~C1~~ | 計算量4倍 | **完了。読みCER 11.98% で新記録**（-1.40pt、有意） |
| ~~T3~~ | flow_copies / condition_dropout | **完了。既定値のままが最良** |
| ~~G1~~ | 停止の健全性の指標化 | **完了**。base 3.2% → 現行 1.2%（有意） |
| ~~M3b~~ | 韻律転写の診断 | **完了。同じ台詞の参照でも韻律を写さない**（経路が無い） |
| **F3** | 新最良を frontend 込みで測る | **次に着手** |
| **M4** | 韻律を条件づける経路を作る | **次（本命）**。構造を変えないと抑揚は動かない |
| S2 | 1,000時間 | **保留を解いた**が、外挿では 1,000h で -1.5pt。抑揚には効かない |
