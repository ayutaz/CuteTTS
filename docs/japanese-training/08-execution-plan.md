...
| **T1** | 学習率の探索 | lr=2e-5 は凍結時代の値。最適値は別か | 要 | **次に着手**。判定指標が3つ揃った（CER / 抑揚 / アクセント） |
...
4. **T1（lr）が次** — 学習17回すべてが `lr=2e-5` 固定で、しかも
   **backbone が凍結していた時期に選んだ値**（[R-025](07-risks-and-decisions.md)）。
   一次のパラメータが未探索のまま規模を上げるのは順序が逆。
   **いま初めて、CER・抑揚・アクセントの3つで判定できる**
5. **S2 は保留** — データ量の効きが最も弱いと実測で分かった（19倍で -1.90pt）。
   T1 / T2 を終えてから、改めて費用対効果で判断する
...
- [ ] **CER・抑揚・アクセントの3つ**で判定している（M1完了により可能になった）
- [ ] 差に信頼区間が付いている
...
| 指標 | 評価set | 検出限界 | 基準 |
|---|---|---|---|
| **読みCER** | `eval_set_v3.json` 600文 | 約1.5pt | 人間の床 5.59% / 現行 13.38% |
| **輪郭の相関** | `prosody_eval_set_v2.json` 240文 | 0.045 | 床 -0.009 / 現行 +0.122 |
| **アクセント核** | 同上（1,491句） | — | 人間対辞書 44.8% / 現行 43.6% |

**CERだけで判定しない。** 読みが直っても抑揚が壊れる可能性があり、逆もある。

### 見積もり

**学習**: 10,000 step × 4水準 ≒ 2 GPU時間（3,000 step で9分の実測から外挿）。

**評価**: 1水準あたり CER（600文）+ 抑揚/アクセント（240文）。
ローカル実測で CER 約2時間 / 抑揚 約1.3時間 → **4水準で13時間**。
これは長すぎるので、次のいずれかを採る。

* **評価setを分割して並列に回す**（推奨）。生成は `batch=1` の自己回帰で
  **GPUを2〜3割しか使えていない**（実測: 使用率15〜71%、消費電力45〜82W /
  TDP285W）。1プロセス 4.2 GiB しか使わないので **3分割で約3倍**
* 先に **CERだけで4水準を篩い、上位2水準だけ抑揚とアクセントを測る**

**費用**: vast.ai RTX 3090 で **$2〜4** の見込み（学習2h + 評価5〜13h、
分割の有無で変わる）。起動前に見積もりを提示する。
