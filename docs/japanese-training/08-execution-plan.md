...
| **G1** | 停止の健全性を指標化する | 文末の繰り返しはどのくらい出ているか | 不要 | **次に着手**。試聴9文中4文で出たが**指標が無い**（R-021 は打ち切りだけを数えていた） |
| **M3** | 抑揚に効く手段を設計する | 何が抑揚を動かすのか | 要 | **次**（まず設計）。lr / batch / 学習対象 / データ量の**すべてで動かなかった** |
| **F2** | frontend の音への効果 | J3→J2 の修正は音に効くか / J3 はアクセントを壊すか | 要（軽） | **提案**。数詞set 200文 + frontend込みの抑揚（約1.5h / 約$0.4） |
| **D2** | データ量の3点目（80h） | 傾き -3.0pt/10倍 は一定か | 要 | **提案**。S2 の前提を確かめる（約3h / 約$0.7） |
...
**済んだ順序（2026-09-16〜17）:** F1 → M2 → D1。3つとも完了した。

* **F1**: 評価は既定で frontend を適用していなかった。実運用の主値は **12.36%**
* **M2**: 天井は **輪郭 +0.38 / アクセント 64.5%**。現行は輪郭で**隔たりの約32%**（R-037 / D-045）
* **D1**: データ量は 30,000 step では梃子（**-3.82pt**）。**ただし抑揚は動かない**（R-038 / D-046）

**これからの順序（2026-09-18）:**

| 順 | ID | 内容 | GPU | 見積もり | 判定（これが出たら次へ） |
|---|---|---|---|---|---|
| 1 | **G1** | 停止の健全性（文末の繰り返し）を指標化し、既存runで測る | **不要** | 約2h / $0 | 繰り返し率が出る。**base と現行で比較できる** |
| 2 | **M3** | 抑揚に効く手段を設計する（まず紙の上） | 不要 | 約2h / $0 | 試す手段が1つ決まる。**天井の68%が未達なのに梃子が無い** |
| 3 | **F2** | 数詞set 200文 + **frontend込みの抑揚**を測る | 要（軽） | 約1.5h / 約$0.4 | R-039 の修正が音に効くか / J3 がアクセントを壊すか |
| 4 | **D2** | データ量の**3点目（80h）**を 30,000 step で測る | 要 | 約3h / 約$0.7 | 傾き -3.0pt/10倍 が一定か。**S2 の前提** |
| 5 | **C1** | 計算量を4倍にする（batch16 × 30,000 step） | 要 | 約6.5h / 約$1.5 | 1pt以上の有意改善 → 最終モデルは長時間学習で作る |
| 6 | T3 | `flow_copies` / `condition_dropout` | 要 | 約4〜8h / 約$1.3 | 有意差が出れば採用 |
| 7 | S2 | 1,000時間 | 要 | **大**（前処理 239 GPU時間 + 学習） | **D2 / C1 の後に費用対効果で判断** |

**3〜5 を1つのインスタンスで続けて回せば 約11時間 / 約$2.6。**
着手前に費用を提示する（D-024）。

**1 と 2 は GPU が要らないので先にやる。** G1 は試聴で最も目立った問題に
指標が無いという穴を埋める。M3 は「抑揚に投資する」と決めた（D-045）のに
**手段が1つも見つかっていない**ので、測る前に設計が必要。
...
- [ ] ASR転写から繰り返しを検出する規則を決める（例: 末尾の n-gram 反復）
- [ ] **既存の metrics.json を再集計する**（生成をやり直さない）
- [ ] base / S0 / 現行 / D1-17h で繰り返し率を比較する
- [ ] 停止の健全性を**CERとは独立したゲート**として08章に書く

### 見積もり

約2時間。**GPU不要**（保存済みの転写を読むだけ）。
...
| 候補 | 考え方 | 懸念 |
|---|---|---|
| **F0 を補助目標にする** | flow matching の loss に韻律項を足す | 学習実装の変更が要る。latentとF0の対応付け |
| **韻律を明示的に条件づける** | アクセント句境界・核位置を入力に足す | prompt形式の変更は推論側と同期が必要 |
| **参照音声の選び方** | 対象文と韻律が近い参照を選ぶ | R-026（長さ）は棄却済み。別の軸で測る |
| **J3 の副作用を外す** | `箸`/`橋` の区別が消える問題（R-039） | F2 で測ってから |

### ゴール

- [ ] 候補を3つ以上挙げ、**実装量と測定可能性**で順位を付ける
- [ ] 1つ選んで、**どう測れば効果が分かるか**を先に決める（M1/M2 の指標で足りるか）
- [ ] 学習が要るなら費用見積もりを出す

### 見積もり

設計だけなら約2時間 / $0。**実装と測定は選んだ手段による。**
...
数詞set 約40分 + 抑揚 約45分 = **約1.5時間 / 約$0.4**。
