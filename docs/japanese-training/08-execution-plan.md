...
| 順 | ID | 内容 | GPU | 見積もり | 判定 |
|---|---|---|---|---|---|
| 1 | **F3** | 新最良（C1）を **frontend 込み**で測り、主値を差し替える | 要（軽） | 約1h / 約$0.3 | 実運用値が出る（11.98% から更に下がる見込み） |
| 2 | **M4a** | **アクセント核をテキストに明示して学習する**（`modeling/` は無変更） | 要 | 実装3h（GPU不要）+ 学習・評価 約6h / 約$1.4 | **アクセントが +5pt 以上動けば、韻律の条件づけは有効** |
| 3 | M4b / M4c | 韻律埋め込み / F0 を補助目標にする（**構造変更**） | 要 | 大 | M4a が動かなかったときに進む |
| 3 | A1 | アクセント専用set（`箸`/`橋` 型の語） | 不要 | 約2h / $0 | J3 がアクセントを壊すかを判定できるようにする |
| 4 | C2 | 計算量をさらに4倍（192万サンプル） | 要 | 約20h / 約$5 | 読みがどこまで下がるか（C1 の続き） |
| 5 | S2 | 1,000時間 | 要 | 大 | データ量の投資。外挿では 11.9% |

**M4 が本命。** 読みは C1 / S2 で下げられるが、聴取で最多だった抑揚は
**構造を変えない限り動かない**ことが9段の実験で確定した。
...
- [ ] 2本を同一条件（30,000 step / batch 4 / lr 2e-5 / seed 42）で学習する
      - (a) **仮名化のみ**（記号なし）
      - (b) **仮名化 + アクセント核の記号**
- [ ] 3指標 + 停止健全性で判定する。`ParameterDrift` を記録する
...
**(a) を置く理由**: (b) だけを走らせると「仮名化の効果」と「記号の効果」が
混ざる。(a) は仮名化だけなので、差分が記号の効果になる。
