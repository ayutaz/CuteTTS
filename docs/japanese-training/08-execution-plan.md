...
| ~~3~~ | ~~A1~~ | アクセント専用set | 要（軽） | 実測 約1h / 約$0.2 | **完了**（R-048） |
| ~~4~~ | ~~M4c~~ | 韻律を構造で受け取る | 要 | 実測 約9h / 約$1.9 | **完了**（R-054 / D-052）。**輪郭は +0.118 制御できるが、与えるものが無いので既定にしない** |
| ~~5~~ | ~~M4b~~ | テキストから韻律を予測 | 不要（CPU） | 実測 数分 | **完了**（R-053）。**辞書は輪郭を持たない**（-0.001）。学習した予測器も +0.113 |
| **1** | **C2** | 計算量4倍（accent × batch16 × 30,000 step） | 要 | 実測 約8h / 約$1.9 | 読みの記録 |
| 2 | **M4d** | **モーラ単位の条件**（12.5 Hz ではなくモーラ境界） | 要 | 中 | **アクセント核が動かない理由の候補。** 1 patch が約1.5モーラなので、モーラ単位の上下を指定できていない |
| 3 | **M4e** | 条件を**外しても劣化しない**ようにする（学習中に条件をdropout） | 要 | 小 | 採用の障害が1つ消える。いまは条件が外れると素のモデルより悪い |
| 4 | S2 | 1,000時間 | 要 | 大 | 外挿では -1.5pt。**抑揚には効かない** |

**韻律転写は機能として成立した**（D-052）。同じ台詞の人間の読みから F0 を
取り出して head の条件へ渡すと写る（+0.118、有意）。M3b で「参照音声からは
写らない」と分かっていたので、**F0 を取り出す一手間が要る**というのが答え。
