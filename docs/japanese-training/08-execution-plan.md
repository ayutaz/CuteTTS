...
| **T2** | batch size / 学習対象 | 他のハイパーパラメータに余地があるか | 要 | **次に着手**。`batch_size=4` / `flow_copies=4` / `condition_dropout=0.1` / 6 module全部を学習 が**いずれも未検証** |
...
5. **T2 が次** — 残る未探索は `batch_size=4` / `flow_copies=4` /
   `condition_dropout=0.1` / **6 module全部を学習していること**。
   head は凍結時代に唯一学習されていたので過適合の可能性がある
6. **S2 は保留** — データ量の効きが最も弱いと実測で分かった（19倍で -1.90pt）。
   T2 を終えてから、改めて費用対効果で判断する
...
- [ ] `batch_size` を 2水準以上（例 4 / 16）で比較している
- [ ] **学習対象**を 2水準以上（全部 / head凍結）で比較している
- [ ] **CER・抑揚・アクセントの3つ**で判定している
- [ ] 差に信頼区間が付いている
- [ ] `ParameterDrift` が全runで記録されている
...
**T1 で lr も梃子でないと分かった**（R-035）ので、残る未探索は T2 の
batch size / 学習対象だけ。**T2 を終えてから、改めて費用対効果で判断する。**

#### 保留を解く条件

次のいずれかが満たされたとき、規模を上げる判断材料になる:

* **T2 で有意な改善が出る** → ハイパーパラメータ側にまだ余地があると分かる
* **T2 でも出ない** → 学習の設定は出尽くしたので、規模かデータの質が残る
* **抑揚・アクセントが規模で伸びる見込みが立つ**（現状は根拠なし）
