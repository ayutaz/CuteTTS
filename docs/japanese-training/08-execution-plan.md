...
| **M2** | 抑揚・アクセントの天井 | +0.122 / 45.3% は天井のどこか | 要（軽） | **提案**。別テイクが 27,367組あると分かった |
...
| 2 | **M2** | 抑揚・アクセントの**天井**を別テイクで測る | 要（軽） | 約1.5h / 約$0.3 | 天井が出る。**+0.30以上なら抑揚に投資、+0.15前後なら抑揚から手を引く** |
...
- [ ] 別テイク対を **150組以上**（話者30人以上）集める
- [ ] 人間 対 人間で **輪郭の相関**と**アクセント核の一致**を測る
- [ ] 現行モデル（+0.122 / 45.3%）を床と天井の間に位置づける
...
### 天井が分かっていない → **測れる見込みが立った（M2）**

同一文・同一話者の人間の**別テイク**が無いので、+0.122 や 43.6% が
到達可能値のどこかは測れない。**床からの距離しか言えない。**

**2026-09-16 追記: gol の metadata を全走査すると、同じ game・同じ話者・
同じ台詞で長さの違う録音が 27,367組（557 game / 4,138話者）あった。**
既存の抑揚set（6 game）の中には2組しかないので別の game の音声が要るが、
**天井は測れる。** → M2
...
* **抑揚・アクセントが規模で伸びる見込みが立つ**（現状は根拠なし。M2 の天井が出れば判断できる）
...
| **抑揚に投資するか** | 未確定 | **M2** | 別テイク同士の一致率（＝天井）。+0.30以上なら投資、+0.15前後なら手を引く |
