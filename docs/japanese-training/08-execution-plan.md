...
| ~~3~~ | ~~**F2 + M3b**~~ | 数詞set + frontend込み抑揚 + 参照差し替え + 韻律転写の診断 | 要（軽） | 実測 約1.2h / 約$0.3 | **完了。数詞 -11.44pt / アクセントは判定不能 / 喋り続けは参照に依存しない / 韻律を写さない**（R-041 / D-047） |
| ~~4~~ | ~~**D2**~~ | データ量の**3点目（80h）**を 30,000 step で測る | 要 | 実測 約2.2h / 約$0.5 | **完了。-3.05 と -3.02pt/10倍 で一致。対数直線**（R-042） |
| ~~5~~ | ~~**C1**~~ | 計算量を4倍にする（batch16 × 30,000 step） | 要 | 実測 約5.9h / 約$1.3 | **完了。-1.40pt で判定を満たした** → 最終モデルは計算量を積む（D-048） |
