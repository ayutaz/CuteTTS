...
| S2 | 1,000時間 | 分布を広げても安定するか | 要 | **保留**。学習設定に余地が無かったので、次は**データ量を 30,000 step で測り直す**（D1、提案） |
...
**次の候補（提案。どれも未着手。GPUを使うので着手前に費用を提示する）:**

| 候補 | 問い | 規模 |
|---|---|---|
| **D1: データ量の再測定** | 17h と 325.9h を **30,000 step** で比べても差は -1.90pt のままか | 学習1本 + 評価（約3h / 約$0.5）。17h 版データの所在を要確認 |
| T3: `flow_copies` / `condition_dropout` | 残る2つの一次パラメータに余地はあるか | 学習2〜4本（約4〜8h） |
| 長時間学習 | 30,000 step の先（収穫逓減の続き） | 学習1本（数時間） |

**推す順は D1 → T3。** D1 は S2 の保留を解く条件（下記）に直接答える。
しかも以前のデータ量比較は 3,000 step で、**大きいデータに不利な条件だったと
自分で認めている**（325.9h は1 epochの5%しか見ていない）。
...
* **T2 でも出ない** → **こちらになった。** 学習の設定はほぼ出尽くしたので、規模かデータの質が残る
...
**大きいデータセットに不利な条件だった。** **30,000 step で測り直すのが次の候補（D1）。**
