...
|---|---|---|---|---|---|
| **1** | **M4f** | **アクセント核が「条件に入っているか」を確かめる** | **不要（CPU）** | 数分 | 12.5 Hz の条件から読んだ核と、元の音声から読んだ核が一致するか。**一致しなければ条件が核を運んでいない**ので、モデルを疑う必要が無くなる |
| 2 | M4g | 条件を**隣接モーラの差分**にする | 要 | 中 | アクセント核は相対関係で決まる。水準ではなく差分を渡す。**M4f で「条件には入っている」と分かってから** |
| 3 | M4h | 韻律転写を**本番データで**回す（325.9h / `--f0-dropout 0.1`） | 要 | 約13h / 約$2.6 | 機能として使える品質になるか。**17.9h の検証しかしていない** |
...
**目的**: アクセント核を動かす。**M4f の結果で中身が決まる。**

* 条件が核を運んでいるなら → **隣接モーラの差分**を渡す
  （アクセント核は水準ではなく**相対関係**で決まる）
* 運んでいないなら → 条件の時間分解能を上げる

**ゴール**: アクセント核（対人間）が **mismatch 比で有意に改善**。
現行は +0.61pt（95%CI [-3.50, +4.64]）で、検出限界は約4pt。
...
**見積もり**: F0 cache（325.9h）約7時間 + 学習 約1.6時間 + 評価 約2時間
= **約11時間 / 約$2.2**。
