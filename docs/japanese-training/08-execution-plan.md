...
| checkpoint | in_domain mean / median |
|---|---:|
...
| 指摘 | 割合 | CERで測れるか | 対処 |
|---|---:|---|---|
| **読み間違い** | **45%** | 部分的に | **J3（読み付与）で直ることを実証済み** |
| 抑揚が不自然 | 40% | **測れない** | 未解明 |
| アクセントが違う | 30% | **測れない** | 未解明 |
...
| ID | フェーズ | 答える問い | 学習 | 状態 |
|---|---|---|---|---|
| **J3** | 読み付与 frontend | 誤読（指摘45%）を frontend で消せるか | **不要** | **次に着手** |
| **J4** | Tokenizer 互換拡張 | byte-fallback を減らすと何が改善するか | 要 | J3の結果を見て |
| **T1** | 学習率の探索 | lr=2e-5 は凍結時代の値。最適値は別か | 要 | T2より先 |
| **T2** | batch size / 学習対象 | 他のハイパーパラメータに余地があるか | 要 | T1の後 |
| **M1** | 抑揚・アクセントの測定 | CERの外にある指摘（40%/30%）をどう測るか | 不要 | 並行 |
| S2 | 1,000時間 | 分布を広げても安定するか | 要 | **保留**（J3/T1の後に再判断） |
...
1. **J3 が最優先** — 指摘の最多（45%）に直接対応し、**再学習が不要**で最も安い。
   J2 が同じ機構で -11.80pt を実証している
2. **T1（lr）が次** — 学習17回すべてが `lr=2e-5` 固定で、しかも
   **backbone が凍結していた時期に選んだ値**（[R-025](07-risks-and-decisions.md)）。
   一次のパラメータが未探索のまま規模を上げるのは順序が逆
3. **M1 は並行** — 抑揚40% / アクセント30% は CER では測れない。
   測る手段が無いままでは、直ったかどうかを判定できない
4. **S2 は保留** — データ量の効きが最も弱いと実測で分かった。
   J3 / T1 を終えてから、改めて費用対効果で判断する
...
- [ ] 任意の日本語テキストから読みを取れる（形態素解析器 + 辞書）
- [ ] **どの語を置換するかの規則が決まっている**。全文を仮名にすると
      学習分布（漢字かな混在）から外れるので、置換は最小限にする
- [ ] 専用の評価setで効果が測られている（J2 と同じく、n を確保して信頼区間を出す）
- [ ] 通常の会話文で**悪化しない**ことが確認されている（J2 は in_domain 600文で +0.02pt）
- [ ] `scripts/synthesize_japanese.py` に組み込まれ、既定で有効

### 設計課題（未確定）

1. **依存関係の追加**。形態素解析器が必要になる。
   `fugashi` + `unidic-lite`（pip、Windows可）、`SudachiPy`、`pyopenjtalk` 等。
   **ユーザー判断を要する**（現在の依存は torch / transformers / soundfile 程度）
2. **置換の対象をどう選ぶか**。候補:
...
`lr=2e-5` が適切かを初めて確かめる。

### 根拠（[R-025](07-risks-and-decisions.md)）

学習17回すべてが **lr=2e-5 固定**で、`batch_size=4` / `flow_copies=4` /
`condition_dropout=0.1` も全runで同じ。変えたのは warmup だけ。

**問題は、2e-5 が backbone 凍結時代に選ばれた値であること。**
当時は実質 fp32 の DiT head（70.5M）だけが学習されていた。
いまは 228.6M 全部が動くので、**最適な lr が同じである理由がない。**

「データ量は弱い、step数は頭打ち」という結論は、
**探索した3軸（dtype / step数 / データ量）の中での話**である。

### ゴール

- [ ] 3〜4水準（例: 1e-5 / 2e-5 / 5e-5 / 1e-4）を同一step数で比較している
- [ ] 評価は **v3（600文）** で、差に信頼区間が付いている
- [ ] `ParameterDrift` の「重みが動いた割合」が全runで記録されている
- [ ] 最良値が 2e-5 以外なら、step数の収穫逓減も測り直す

### 見積もり

10,000 step × 4水準 + v3評価4回 ≒ 6〜7 GPU時間・**約$1.5**（vast.ai）

---

## T2: batch size / 学習対象

T1 の後。`batch_size=4` 固定、6 module すべてを学習中という2点が未検証。
head は凍結時代に唯一学習されていたので過適合の可能性がある。
...
**CERの外にある指摘を測れるようにする。** 聴取で抑揚40% / アクセント30%の
指摘があったが、どちらも CER では測れず、日本語向けの信頼できる自動指標も無い。
**測る手段が無いままでは、直ったかどうかを判定できない。**

### 候補

1. **聴取protocolの定型化**。現在の聴取キットは都度作っている。
   盲検A/B・アンカー（人間の実音声）・書き出しを固定手順にする。
   `scripts/build_listening_kit.py` が土台
2. **F0 contour の比較**。同一話者・同一文の人間音声があるので、
   log-F0 を z 正規化して DTW で形を比べられる。
   `torchaudio.functional.detect_pitch_frequency` で追加依存なし
3. **高音域の評価**。聴取で「高音域の発音がつぶれる」指摘が1件あった。
   **P1c は高音域を評価していない**（log-mel L1 / SNR / speaker cos のみ）。
   VAE往復で帯域別に測れば、S4（Japanese VAE）の再判断材料になる

### 注意

**アクセントは信頼できる自動測定が現実的に存在しない。** 最小対（橋/箸/端）の
聴取で代替すると n が小さく統計的な主張はできない。
**ゴールの文言を「聴取による定性確認」へ書き換える判断が要る。**
...
|---|---:|
| dtype修正 | -15.77pt |
| J2 読み展開（数詞） | -11.80pt（学習不要） |
| step数 3,000 → 30,000 | -4.20pt |
| **データ量 17h → 325.9h（19倍）** | **-1.90pt** |

19倍のデータで -1.90pt に対し、frontend は学習なしで -11.80pt を出した。
**J3（読み付与）と T1（lr探索）を終えてから、改めて費用対効果で判断する。**

ただしデータ量の比較には**自分で認めた限界がある**。比較は 3,000 step で行い、
325.9h はそこで1 epochの5%しか見ていない（17h はほぼ1 epoch）。
**大きいデータセットに不利な条件だった。** T1 の後に 30,000 step で測り直す価値がある。
