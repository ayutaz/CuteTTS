...
| ID | フェーズ | 答える問い | 学習 | 状態 |
|---|---|---|---|---|
| **J3** | 読み付与 frontend | 誤読（指摘45%）を frontend で消せるか | **不要** | **実装・測定済み**（読みCER -4.23pt。会話文の確認のみ残） |
...
| **M1** | 抑揚・アクセントの測定 | CERの外にある指摘（40%/30%）をどう測るか | 不要 | **抑揚の測定器は完成**（床 +0.08 / 67ペア）。**アクセントは未達**（強制アラインメントが要る） |
...
2. **T1（lr）が次** — J4 は J3 の結果を見て**見送りにした**（D-039 / R-030）。
   学習17回すべてが `lr=2e-5` 固定で、しかも
...
- [ ] 通常の会話文で**悪化しない**ことが確認されている（J2 は in_domain 600文で +0.02pt）
      — 測定中（`v3-30000-plain` / `v3-30000-yomi`）
