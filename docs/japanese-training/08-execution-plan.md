...
| 順 | ID | 内容 | GPU | 見積もり | 判定 |
|---|---|---|---|---|---|
| ~~1~~ | ~~F3~~ | 新最良を frontend 込みで測る | — | — | **M4a-split が上書き**（7.58%） |
| ~~2~~ | ~~M4a~~ | アクセント核をテキストに明示 | 要 | 実測 約6h / 約$1.3 | **完了**（R-045 / D-050 → D-051 で訂正） |
| ~~3~~ | ~~A1~~ | アクセント専用set | 要（軽） | 実測 約1h / 約$0.2 | **完了**（R-048）。**前提は否定された**（J3 は常用語を置換しない）が、**記号の価値を測る器になった** |
| **1** | **M4b / M4c** | **韻律を受け取る経路を作る**（構造変更） | 要 | 大 | **本命。** 抑揚は9軸 + 参照 + テキスト記号のすべてで人間に近づかない |
| 2 | C2 | 計算量4倍（accent × batch16 × 30,000 step） | 要 | 約4.7h / 約$1.0 | 読みの記録。**聴取での指摘は抑揚とアクセント**なので効用は小さい |
| 3 | S2 | 1,000時間 | 要 | 大 | 外挿では -1.5pt。**抑揚には効かない** |
...
| 段 | 内容 | 見積もり | 根拠 |
|---|---|---|---|
| 1 | latent → 波形（VAE decoder。GPU） | 325.9h を 40×実時間として **約8 GPU時間 / 約$1.7** | **未実測**。P1e の encode 側 44.5× からの外挿。**最初に測る** |
| 2 | 波形 → F0 @12.5 Hz（`pyworld`。CPU） | **8コアで約9.3時間** | **実測**（12件71.7秒）。`harvest`+`stonemask` は **4.4×実時間/コア**、`dio` は 25.6× |
| 3 | F0 cache（latent cache と同じ形で並べる） | 数GB | — |

**F0 は `harvest` を使う**（`dio` の方が5.8倍速いが、測定側
`prosody.track_f0` が `harvest`+`stonemask`+局所ジャンプ補正なので、
**条件づけと測定で推定器を揃える**）。段1（GPU）と段2（CPU）は
**同時に流せる**ので、実時間は max(8h, 9.3h) ≒ **約10時間**。

**まず 17.9h の部分集合で経路を検証する**（D1 で使ったもの）。
decode 約0.5 GPU時間 + F0 約30分（8コア）で、
**約$0.2 で経路の可否が分かる**。
...
| 与え方 | F0 の出どころ | 意味 | 判定 |
|---|---|---|---|
| **oracle** | **その文自身の人間音声** | 経路が通っているかの検査 | 輪郭の相関 **+0.30 以上**（現行 +0.091、天井 +0.382）。**届かなければ実装を疑う** |
| **transfer** | **同じ台詞の別テイク**（M2 の 200対） | 実際に使える形での上限 | 天井 +0.382 に対してどこまで行くか |
