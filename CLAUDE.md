...
**P0 / P1 / P2 / S0 / S1 完了。J2 / J3 / M1 / T1 / T2 も完了。
**F1 / M2 / D1 / G1 / M3 / F2 / M3b / D2 / C1 / T3 / M4a 完了（2026-09-18）。**

**最良checkpointは `m4a-accent`。読みCER 7.58%**（従来 13.38%、-5.80pt）。
**人間の床 5.59% まで 2.0pt。** 学習と推論の両方で**全文を片仮名にして
アクセント核に記号を置く**（`--frontend accent`。R-045 / D-051）。

**読みCERで効いているのは片仮名化だけ**（M4a-split / R-047）。内訳は
「frontend を学習と揃える -1.74pt」+「**全文片仮名化 -4.40pt**」で、
記号を足す効果は +0.34pt（有意差なし）、**核を偽にしても変わらない**（R-046）。

**それでも記号を入れる。** 外すと**同音異義のアクセントを一切作り分けられない**
（A1 / R-048。対の区別 75.0% → **0.0%**）。**読みCERは原理的にこの誤りを
見ない**（`箸` も `橋` も読みは `ハシ`）ので、読みで差が無いなら区別を保つ方を採る。
依存を最小にしたいときは `m4a-kanafull`（`kana_full`、7.24%）。

**抑揚は動かない。** モデルは記号の位置には従う（偽の核にすると
対辞書が -5.95pt 動く）が、**人間との一致は変わらない**（M2 と同じ壁）。

**学習設定の一次パラメータは出尽くした**（lr / batch / 学習対象 /
`flow_copies` / `condition_dropout` すべて梃子でない）。
**残る梃子は計算量とデータ量だけ**で、どちらも読みには効くが抑揚には効かない。

**最良checkpointが更新された: C1（batch16 × 30,000 step = 48万サンプル）で
読みCER 11.98%**（従来 13.38%、-1.40pt、有意。R-043 / D-048）。
計算量はまだ梃子だった。喋り続けも 1.2% → 0.5%。**抑揚は動かない。**

**抑揚は「韻律を条件づける経路」を作らないと動かない**（D-047 / R-041）。
同じ台詞の別テイクを参照に渡しても写さなかった（+0.160 対 天井 +0.382）。
声質と声域は取るが、時間的な輪郭は取らない。

現行の最良は **`m4a-accent`**（`--frontend accent` が要る）。3指標での位置（`frontend accent` 以外は S1v2 fp32 30,000 step の値）:
...
- **評価スクリプトは既定で frontend を適用しない。** `--expand-numerals`（J2）と
  ...
  **実運用の現行最良は 12.36%**（frontend込み）。比較用の 13.38% と混同しない（F1）。
...
### 未探索の手段（2026-09-16 時点）

**探索したのは dtype / step数 / データ量 / 学習率 / batch size / 学習対象 の6軸。**
学習24回すべてで `flow_copies=4` / `condition_dropout=0.1` が固定のまま。
**「データ量は弱い、step数は頭打ち」はこの6軸の中での結論。**
T2 で分かったのは、**効いているのは batch の大きさではなく計算量**だということ
（batch16・10,000 step と batch4・30,000 step は区別できない。R-036）。
...
| **全文仮名化 + 核記号（M4a）** | **-4.06pt**（新記録 7.58%） | 要（済） |
...
| **F3** | 新最良を frontend 込みで測る | **次**。C1 の 11.98% は frontend 無しの値 | **次** |
| **M4** | 韻律を条件づける経路を作る | **次（本命）**。抑揚は7軸すべてで動かず、参照からも読めない。**構造を変える唯一の道** | **次** |
...
| **C1** | 計算量4倍 | 効いているのは計算量。30,000 step の先に 1pt 以上が残っているか | 提案 |
| T3 | flow_copies / condition_dropout | 引数は既にある。期待値は低い | 提案 |
...
| F2 | frontend の音への効果 | R-039 の修正が音に効くか / J3 がアクセントを壊すか | 提案 |
| D2 | データ量の3点目（80h） | 傾き -3.0pt/10倍 が一定か。S2 の前提 | 提案 |
| S2 | 1,000時間 | **保留を解いた**（D-046）が、外挿では **1,000h で -1.5pt / gol全体でも床の +3.2pt 手前**。抑揚には効かない | 着手可（費用対効果で判断） |
