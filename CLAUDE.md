...
**最良checkpointは `m4a-kanafull`。読みCER 7.24%**（従来 13.38%、-6.14pt）。
**人間の床 5.59% まで 1.65pt。** 学習と推論の両方で**全文を片仮名にする**
（`--frontend kana_full`。R-047 / D-051）。

**アクセント記号は寄与していなかった**（M4a-split）。-5.80pt の内訳は
「frontend を学習と揃える -1.74pt」+「**全文片仮名化 -4.40pt**」で、
記号を足す効果は +0.34pt（95%CI [-0.25, +1.00]、有意差なし）。
**核を偽の位置に置いても読みCERは変わらない**（-0.18pt、R-046）。
`kana_full` は full-context label が要らないので**単純で、かつ最も安定**。
