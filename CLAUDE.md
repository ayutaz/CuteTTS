...
**→ 経路は作れた**（M4c / R-054）。**参照音声から F0 を取り出して
DiT head の adaLN へ渡すと輪郭が写る**（対照比 +0.118、有意）。
**ただし既定にはしない**（D-052）。実運用では与えるものが無く、
テキストからの予測は実測と +0.113 しか相関しない（M4b / R-053）。
**条件が外れると素のモデルより悪い**ので、いま既定にすると品質を下げる。
...
**読みで残るのは計算量とデータ量だけ**で、どちらも**抑揚には効かない**。
**抑揚は11通りの手段すべてで動かなかった**（上記 + 参照の与え方 +
テキストへの記号）。**残るのは構造を変えること**（M4c）。
...
| **M4d** | モーラ単位の条件 | **次**。1 patch が約1.5モーラなので、**モーラ単位の上下を指定できていない**のがアクセントが動かない理由の候補 | 提案 |
| **M4e** | 条件を外しても劣化しないようにする | 学習中に条件をdropoutする。**採用の障害が1つ消える** | 提案 |
...
| **C2** | 計算量4倍（accent × batch16 × 30,000 step） | 読みの記録。約4.7h / 約$1.0。**聴取での指摘は抑揚とアクセント**なので効用は小さい | 提案 |
