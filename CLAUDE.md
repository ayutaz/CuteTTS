...
**最良checkpointは `m4a-accent`。読みCER 7.58%**（従来 13.38%、-5.80pt、有意）。
**人間の床 5.59% まで 2.0pt。** アクセント核をテキストに明示して学習した
（`--frontend accent`。R-045 / D-050）。**推論も同じ frontend が要る。**

**抑揚は狙って外した。** アクセントは +0.93pt で有意差なし。
記号どおりに読むようにはなった（対辞書 39.4% → 47.1%）が、
**人間との一致は上がらない**（M2 と同じ壁）。
