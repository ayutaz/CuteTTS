...
**F1 / M2 / D1 も完了。次は G1（停止の健全性の指標化）と M3（抑揚の手段の設計）**。
どちらも GPU不要。その後 F2 → D2 → C1（08章「これからの順序」）。
...
- **文末の繰り返しは打ち切りとは別の失敗。** 試聴9文中4文で
  「文が終わった後に発話を続ける」が出た（`ぶっ込ませんか`）。
  R-021 の打ち切り勘定には出ないが CER に入る。**停止の健全性は別に数える。**
...
| **F1** | 評価を実運用と揃える | **次**。評価は既定で J2/J3 を適用していない。実運用の現行最良は **読みCER 12.36%** | **次** |
...
| **G1** | 停止の健全性 | 試聴9文中4文で**文末の繰り返し**が出たのに指標が無い。既存の転写を再集計する（GPU不要） | **次** |
| **M3** | 抑揚の手段の設計 | 天井の68%が未達なのに、**試した軸すべてで動かなかった**。手段を設計する（GPU不要） | **次** |
