...
M4e / M4f / M4g / M4h も完了（2026-09-19〜20）。**M4 は完了した。**
**D3 で読みも閉じた**（2026-09-21。R-061 / D-055）。**S2 は取り下げ。**
...
| **データ量18倍**（D3） | **-0.59pt（有意差なし）** | **頭打ち**（R-061 / D-055） |

**frontend も計算量もデータ量も出尽くした。読みは閉じた**（D-055）。

**D3 が S2 の根拠を消した。** データ量の傾きは frontend 無しの水準では
-3.0pt/10倍 だったが、**`accent` の水準では -0.47pt/10倍（約1/6）**。
**信頼区間の最も都合のよい端で計算しても** 1,000h は -0.55pt で、
床 5.59% までなお 1.44pt 残る（床には約19,300時間が要り、
**gol corpus 全体 10,466h でも足りない**）。

データ量が治していた誤りは**漢字の読みの引き当て**で、
**全文片仮名化が消す誤りと同じもの**だった。**C2（計算量）と同じ形。**
**残る約2.0pt が何なのかは未解明だが、規模では動かない。**
...
（M4d / R-056）。原因は**条件ではなくモデル**だった —
**12.5 Hz へ畳んでも核は 86.4% 残る**（M4f / R-058）のに、
**モデルは条件に4割しか追随していない**（oracle の輪郭 +0.13 / 上限 +0.367）。

**追随は受け取り側では強まらない**（M4g / R-059 / D-054）。条件づけの層を
線形から MLP 128 にし、学習率を5倍にしても**追随度は +0.126 → +0.133
（95%CI [-0.027, +0.042]、有意差なし）**。一方で条件なしの長さは
5.11 → 4.63秒 と有意に変わったので、**層は強く学習されている＝容量不足ではない**。
**受け取り側の路線はここで閉じた**が、**データ量は梃子だった**（M4h / R-060）。

**韻律転写は本番データ（325.9h）で成立した**（M4h / R-060。checkpoint は
`checkpoints/m4h-prosody/inference/`）。事前に決めた3つの基準をすべて満たした —
**条件の利得 +0.104**（有意）/ **読みCER 7.12%**（7.58% から悪化なし）/
**条件なしの代償 -0.025**（有意差なし）。
**追随度はデータ量で上がる**（17.9h +0.126 → 325.9h **+0.168**、有意。
上限 +0.367 の 34% → **46%**）が、18倍にして1.33倍なので対数的。

**アクセント核だけは最後まで動かなかった。** oracle 43.5% 対 条件なし 43.1%
（有意差なし）。間違った F0 は -3.78pt 壊す（有意）ので**特異性はある**が、
**正しくしても良くならない**。

**M4g の事前基準は作り方が悪かった** — 「mismatch 比で有意に改善」は
**mismatch が壊れるだけで通る**。**条件の効果は oracle 対 none で見る。**
...
| M4f | 完了 | **条件は核を運んでいる**（12.5 Hz へ畳んでも 86.4%）。**モデルの追随が4割**（R-058） |
| M4g | 完了 | **追随は受け取り側では強まらない**（+0.126 → +0.133、有意差なし。R-059 / D-054）。容量・学習率は梃子でない |
| M4h | 完了 | **韻律転写は本番データで成立**（R-060）。3基準すべて達成。**追随度はデータ量で上がる**（+0.126 → +0.168）。アクセント核は動かず |
...
| **データ量 17.9h → 325.9h（19倍・30,000 step）** | **-3.82pt**（読みCER） | 要（済） |
| **データ量18倍（D3。`accent` の水準）** | **効果なし**（-0.59pt、有意差なし） | 要（済） |
...
| 条件づけの容量と学習率（M4g） | **効果なし**（追随度 +0.126 → +0.133、有意差なし） | 要（済） |
| **データ量（17.9h → 325.9h）の追随度への効果（M4h）** | **+0.043**（追随度 +0.126 → **+0.168**、有意） | 要（済） |
...
| ~~M4f~~ | アクセント核が条件に入っているか | **完了**（R-058）。**12.5 Hz へ畳んでも核は 86.4% 残る**＝条件は運んでいる。**モデルの追随が4割**（oracle の輪郭 +0.13 / 上限 +0.367）だと分かり、疑う先がモデル側に確定した | 完了 |
| ~~M4g~~ | 追随を強める（容量 + 学習率） | **完了。動かなかった**（R-059 / D-054）。MLP 128 + 学習率5倍で追随度 +0.126 → **+0.133**（95%CI [-0.027, +0.042]）。条件なしの長さは有意に変わるので**容量不足ではない**。**受け取り側の路線は閉じる** | 完了 |
| ~~M4h~~ | 韻律転写を**本番データ**で回す | **完了**（R-060）。325.9h / 30,000 step / 運用点。**3つの基準すべて達成** — 利得 +0.104 / 読みCER 7.12% / 代償 -0.025。**追随度はデータ量で上がった**（+0.126 → +0.168、有意）。**アクセント核は動かない**（oracle 対 none で +0.37pt） | 完了 |
| ~~D3~~ | 7%の水準でのデータ量 | **完了。梃子ではなかった**（R-061 / D-055）。18倍で -0.59pt、95%CI [-1.41, +0.27]。傾きは -3.0 → **-0.47pt/10倍** | 完了 |
| ~~S2~~ | 1,000時間 | **取り下げ**（D-055）。CI の最も都合のよい端でも -0.55pt で床に届かない。抑揚にも元から効かない | 取り下げ |
