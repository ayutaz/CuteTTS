...
M4e / M4f / M4g も完了（2026-09-19〜20）。M4h（本番データでの韻律転写）が実行中。**
...
**F0 を条件にする方式はここで頭打ち**で、残るのは条件の**入り方**を変えること
（cross-attention / flow の各 step へ繰り返し）＝アーキテクチャの変更。
...
| M4g | 完了 | **追随は受け取り側では強まらない**（+0.126 → +0.133、有意差なし。R-059 / D-054）。**方式の限界** |
...
| **M4h** | 韻律転写を**本番データ**で回す | **実行中**（2026-09-20）。325.9h / 30,000 step / 運用点（線形・`--f0-lr 2e-4` / dropout 0.1 / 先読み4 / head 差込 / 位置あり）。約12.5h / 約$2.2 | **実行中** |
