...
**F1 / M2 / D1 / G1 完了、M3 は設計済み。
いま F2 + M3b → D2 → C1 → T3 を1インスタンスで実行中**（08章「これからの順序」）。
...
| ~~M3~~ | 抑揚の手段の設計 | **設計済み**。**韻律はテキストから決まらない**（人間同士でも +0.38、下位10%は 0.00）。モデルは条件付き平均を出している疑い。決定的な診断 **M3b**（参照＝同じ台詞の別テイク）を用意した | 設計済み |
