...
**`checkpoints/m4a-kanafull/inference/`**（M4a-split。**全文片仮名・記号なし**で
30,000 step）。読みCER **7.24%**（従来の `s1v2-fp32-30000` は 13.38%。
**-6.14pt**。R-047 / D-051）。**人間の床 5.59% まで 1.65pt。**

**推論時も同じ frontend が要る**（`--frontend kana_full`）。素の漢字テキストを
渡すと分布外になる。`kana_full` は**全文を片仮名にするだけ**で、記号は入れない。

**アクセント記号は入れない。** 記号あり（`accent` 7.58%）との差は
+0.34pt / 95%CI [-0.25, +1.00] で**有意差なし**、核を偽の位置にしても
差が出ない（R-046）。`kana_full` の方が**単純**（full-context label が要らない）で
**安定**（自己反復 0.3% / 発散 0 / 超過最大14字。`accent` は 1.0% / 1 / 43字）。

他に残してある checkpoint:

| | 読みCER | frontend | 用途 |
|---|---:|---|---|
| `m4a-accent/inference/` | 7.58% | `accent` | 記号あり（ローカル退避済み） |
| `m4a-shuffled` | 7.40% | `accent_shuffled` | 偽の核の対照（R-046） |
| `m4a-kana` | 11.64% | `yomi` | J3+J2 だけの対照 |
