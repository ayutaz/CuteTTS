...
**韻律転写（M4c）の条件の重みは `checkpoints/m4c-conditioners/` にある**
（`head4.safetensors` が最良: `inject=head` / 先読み4 patch / 16→256）。
**単体では使えない** — 一緒に学習した TTS の重み（17.9h / 10,000 step）は
持ち帰っていないので、使うには学習し直しが要る（約$0.5）。
実験の記録として残してある。
