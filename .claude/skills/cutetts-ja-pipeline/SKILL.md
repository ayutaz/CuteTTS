...
**`checkpoints/m4a-accent/inference/`**（M4a。アクセント核をテキストに明示して
30,000 step。ローカル退避済み、`strict=True` でロード確認済み）。

読みCER **7.58%**（従来の `s1v2-fp32-30000` は 13.38%。**-5.80pt**、有意。R-045）。
`phonetic` は 26.99% → **2.43%**。**人間の床 5.59% まで 2.0pt。**

**推論時も同じ frontend が要る**（`--frontend accent`）。素の漢字テキストを
渡すと分布外になる。`accent` は全文を片仮名にしてアクセント核に `'` を置く。

他に残してある checkpoint:

| | 読みCER | 用途 |
|---|---:|---|
| `c1-batch16-30k/inference/` | 11.98% | 計算量4倍（R-043）。frontend なし |
| `s1v2-fp32-30000/` | 13.38% | 3指標の基準線（M1 / M2 / T1 / T2 / D1 / D2 がこれ） |
