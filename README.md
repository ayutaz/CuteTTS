...
```bash
uv run hf download ayousanz/CuteTTS-jp --local-dir ./model/CuteTTS-jp
...
**渡したテキストは、そのモデルが学習したときと同じ表記へ自動で書き換わります。**
公開しているモデルは「全文を片仮名にして、アクセントの山の位置に記号を付ける」
表記で学習しているので、こう変換されます。

```
frontend=accent
入力: 価格は千二百八十円、消費税込みです。
  → カカクワセ'ンニヒャクハチジュ'ーエン、ショーヒゼ'ーコミデス。
```

**この書き換えを外すと精度が大きく落ちます。** 学習したときと表記を揃えるだけで
読み間違いが **1.74 ポイント**減り、全文を片仮名にすることで **4.40 ポイント**
減りました。どの表記で学習したかは各モデルの `config.json` に記録されているので、
**ふつうは何も指定しなくて構いません**（`--frontend` で上書きできます）。

書き換えは3つのことを同時にやります。

- **読み方を確定させる** — 漢字のままだとモデルが読みを引けないことがあります。
  語彙に無い漢字は内部で細切れのバイト列になってしまいます。
  片仮名にすればこの問題が消えます
- **漢数字を読みに開く** — `千二百八十円` が `セ'ンニヒャクハチジュ'ーエン` に
  なります。学習データに桁の組み合わさった漢数字は 0.32% しか含まれておらず、
  モデルは桁を合成する規則を覚えられません。**仮名なら元から読めます**
- **同じ読みで意味が違う語を書き分ける** — `ハシ'`（橋）と `ハ'シ`（箸）のように、
  記号の位置で区別します。**読み間違いの数は変わりませんが、この区別を
  与えるのは記号だけです**（区別できた割合 0.0% → 75.0%）

古いモデルには `config.json` に記録がありません。その場合は上の `accent` を
当てたうえで、そのことを画面に表示します。別の表記で学習したモデルなら
`--frontend` で指定してください（`none` で書き換えなし）。
