...
#: **長音化を許す**語の品詞。ここに無い品詞の語頭では長音化を止める。
#:
#: 語境界で一律に止めると `でしょう`（NJD では `でしょ` + `う`（助動詞））が
#: `デショオ` になる（正しくは `デショー`）。逆に助動詞以外まで許すと
#: 助詞の `を` が飲み込まれて `ことを` が `コトー` になる（正しくは `コトオ`）。
#: **どちらも実測で見つけた。** 助動詞だけを許すのが両方を満たす。
#:
#: | 規則 | `でしょうか` | `ことを` |
#: |---|---|---|
#: | 語境界すべてで止める | `デショオカ` ✗ | `コトオ` ○ |
#: | 自立語だけで止める | `デショーカ` ○ | `コトー` ✗ |
#: | **助動詞だけ許す**（採用） | **`デショーカ` ○** | **`コトオ` ○** |
_MERGE_POS = frozenset({"助動詞", "記号", "フィラー", "その他"})


def _word_start_moras(text: str) -> set[int]:
    """**長音化を止めるモーラの位置**（0始まり・文全体の通し番号）。
    ...
    **助動詞は除く**（:data:`_MERGE_POS`）。`でしょう` は `でしょ` + `う`
    （助動詞）なので、そこで止めると `デショオ` になってしまう
    （正しくは `デショー`）。逆に助詞まで除くと `ことを` が `コトー` になる。
    ...
    for item in features:
        ...
        if size <= 0:
            continue
        if (item.get("pos") or "") not in _MERGE_POS:
            starts.add(total)
        total += size
    return starts
